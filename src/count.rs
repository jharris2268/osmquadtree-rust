...
pub fn run_count(fname: &str, use_primitive: bool, numchan: usize, filter_in: Option<&str>) -> Result<()> {
    ...
    if fname.ends_with(".osc") {
        ...
        if numchan == 0 {
            ...
            let mut ccs: Vec<Box<dyn CallFinish<CallType=(usize,FileBlock),ReturnType=crate::utils::Timings<Count>>>> = Vec::new();
            for _ in  0..numchan {
                if use_primitive {
        ...
        let mut pps: Vec<Box<dyn CallFinish<CallType=(usize,Vec<FileBlock>),ReturnType=crate::utils::Timings<Count>>>> = Vec::new();
        
        if use_primitive { 
            pb.set_message(&format!("count blocks combine primitive {}, numchan={}", fname,numchan));
            for _ in 0..numchan {
                let cca = Box::new(CountPrim::new());
                pps.push(Box::new(Callback::new(make_read_primitive_blocks_combine_call_all(cca))));
            }
        } else {
            pb.set_message(&format!("count blocks combine minimal {}, numchan={}", fname,numchan));
            for _ in 0..numchan {
                let cca = Box::new(CountMinimal::new());
                pps.push(Box::new(Callback::new(make_read_minimal_blocks_combine_call_all(cca))));
            }
        }
