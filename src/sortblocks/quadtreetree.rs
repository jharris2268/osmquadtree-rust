...
impl fmt::Display for QuadtreeTreeItem {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "{:19}: {:10} total {:10} weight {} children",
            self.qt.as_string(),
            self.total,
            self.weight,
            nc
...
impl<'a> QuadtreeTree {
    ...
    pub fn clone(&self) -> QuadtreeTree {
        QuadtreeTree {
            items: self.items.iter().map(|x| *x).collect(),
            count: self.count,
        }
    }
    ...
    pub fn add(&'a mut self, qt: &Quadtree, w: u32) -> &'a QuadtreeTreeItem {
        ...
        for i in 0..qt.depth() {
            ...
            if self.items[ti].children[v] == u32::MAX {
                ...
                self.items
                    .push(QuadtreeTreeItem::new(qt.round(i + 1), ti as u32));
...
pub fn find_tree_groups(
    mut tree: Box<QuadtreeTree>,
    target: i64,
    absmintarget: i64,
