...
fn read_quadtreeblock_tiles_send(qtsfn: String, send: mpsc::SyncSender<Vec<(ElementType,i64,Quadtree)>>) {
    ...
    for fb in ReadFileBlocks::new(&mut fobj) {
        if fb.block_type=="OSMData" {
            ...
            cc.extend(bl.relations.iter().map(|(a,b)| { (ElementType::Relation,*a,*b) }));
...
impl ChannelQuadtreeBlockFlatIter {
    ...
    pub fn new(qtsfn: &str) -> ChannelQuadtreeBlockFlatIter {
        ...
        /*let jh =*/ thread::spawn( move || read_quadtreeblock_tiles_send(qtsfn, s));
        ChannelQuadtreeBlockFlatIter{/*jh:jh,*/recv:rx.clone(),hadfirst:false,curr:None,idx:0}
    }
...
pub fn make_unpackprimblock<T: CallFinish<CallType=PrimitiveBlock,ReturnType=Timings>>(out: Box<T>)
    -> Box<impl CallFinish<CallType=(usize,FileBlock),ReturnType=Timings>> {
    
    let conv = Box::new(|(i,fb): (usize,FileBlock)| {
        if fb.block_type == "OSMData" {
            PrimitiveBlock::read(i as i64, fb.pos, &fb.data(), false, false).expect("failed to read")
        } else {
