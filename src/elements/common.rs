...
pub fn pack_head(id: &i64, info: &Option<Info>, tags: &Vec<Tag>, res: &mut Vec<u8>,  pack_strings: &mut Box<PackStringTable>) -> Result<()> {
    write_pbf::pack_value(res, 1, *id as u64);
    if !tags.is_empty() {
        write_pbf::pack_data(res, 2, &write_pbf::pack_int(tags.iter().map( |t| { pack_strings.call(&t.key) } )));
        write_pbf::pack_data(res, 3, &write_pbf::pack_int(tags.iter().map( |t| { pack_strings.call(&t.val) } )));
    }
    match info {
        Some(info) => { write_pbf::pack_data(res, 4, &info.pack(pack_strings)?);},
        None => {}
    }
    Ok(())
}
...
pub fn common_cmp(
    left_id: &i64, left_info: &Option<Info>, left_changetype: &Changetype,
    right_id: &i64, right_info: &Option<Info>, right_changetype: &Changetype) -> Ordering {
        
    let a = left_id.cmp(right_id);
    if a != Ordering::Equal { return a; }
    
    let b = left_info.as_ref().unwrap().version.cmp(&right_info.as_ref().unwrap().version);
...
pub fn common_eq(
    left_id: &i64, left_info: &Option<Info>, left_changetype: &Changetype,
    right_id: &i64, right_info: &Option<Info>, right_changetype: &Changetype) -> bool {
    
    if left_id!=right_id { return false; }
    
    if left_info.as_ref().unwrap().version != right_info.as_ref().unwrap().version { return false; }
