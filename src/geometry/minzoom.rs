...
fn get_type(t: &str, line: &usize) -> Result<usize> {
    if t=="0" { Ok(0) }
        ...
        Err(Error::new(ErrorKind::Other,format!("wrong type at line {}", line)))
    }
}
...
fn test_val(vals: &BTreeMap<Tag,(i64,String)>, kv: &Tag) -> Option<i64> {
    ...
    match vals.get(&kv) {
        Some((m,_)) => Some(*m),
        None => {
            match vals.get(&Tag::new(kv.key.clone(),String::from("*"))) {
                Some((m,_)) => Some(*m),
                None => None,
            }
        }
    ...
    pub fn from_reader<R: Read>(min_area: f64, max_minzoom: Option<i64>, reader: R) -> Result<MinZoomSpec> {
        ...
        for (line,row) in csv::Reader::from_reader(reader).records().enumerate() {
            match row {
                Ok(rec) => {
                    if rec.len()!=5 {
                        return Err(Error::new(ErrorKind::Other, format!("minzoom at line {} wrong length", line)));
                    }
                    ...
                    let zoom: i64 = match rec[3].parse() {
                        Ok(p) => p,
                        Err(_) => { return Err(Error::new(ErrorKind::Other, format!("minzoom at line {} zoom not int", line))); },
                    };
                ...
                Err(e) => {
                    return Err(Error::new(ErrorKind::Other, format!("{:?} at line {}", e, line)));
                }
            }
        }
        Ok(res)
    ...
    where T: CallFinish<CallType=WorkingBlock, ReturnType=Timings>
{
    pub fn new(out: Box<T>, spec: Option<MinZoomSpec>) -> FindMinZoom<T> {
        FindMinZoom{out: out, spec: spec, tm: 0.0, na: 0}
    }
    ...
    where T: CallFinish<CallType=WorkingBlock, ReturnType=Timings>
{
    ...
    fn finish(&mut self) -> Result<Timings> {
        ...
        if !self.spec.is_none() {
            ...
            tms.add_other("FindMinZoom", OtherData::Messages(vec![format!("found {} minzooms", self.na)]));
        }
