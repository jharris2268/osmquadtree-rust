...
impl Locations {
    pub fn new(style: Arc<GeometryStyle>) -> Locations {
        Locations{tiles: BTreeMap::new(),max_len:0, max_locs: 0, num_locs: 0,style:style }
    }
    ...
    pub fn get_locs(&self, refs: &Vec<i64>) -> Result<Vec<LonLat>> {
        ...
        for i in refs {
            match self.get_loc(i) {
                Some(l) => res.push(l),
                None => { return Err(Error::new(ErrorKind::Other, format!("missing node {}", i))); }
            }
        }
        Ok(res)
    }
...
impl std::fmt::Display for Locations {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Locations[{} tiles [{} max], {} locations [{} max]", self.tiles.len(), self.max_len, self.num_locs, self.max_locs)
    }
}
    ...
    where T: CallFinish<CallType=WorkingBlock, ReturnType=Timings>
{
    ...
    fn finish(&mut self) -> Result<Timings> {
        ...
        tms.add_other("CollectWayNodes", OtherData::Messages(vec![self.locs.to_string()]));
        if !self.errs.is_empty() {
            tms.add_other("CollectWayNodes", OtherData::Errors(std::mem::take(&mut self.errs)));
        }
        Ok(tms)
    }
}
