use geos_sys::*;
use crate::geometry::{PointGeometry, LinestringGeometry, SimplePolygonGeometry, ComplicatedPolygonGeometry,XY, PolygonPart};
...
unsafe fn make_coords<Iter: Iterator<Item=XY>>(handle: GEOSContextHandle_t, len: usize, pts: Iter) -> *mut GEOSCoordSequence {
...
unsafe fn from_complicatedpolygon_part(handle: GEOSContextHandle_t, part: &PolygonPart) -> Result<*mut GEOSGeometry> {
    
    
    let coords = make_coords(handle, part.exterior.len(), part.exterior.lonlats_iter().map(|l| { l.to_xy(true) }));
    ...
    if res.is_null() {
        return Err(Error::new(ErrorKind::Other, "GEOSGeom_createPolygon_r failed"));
    }
    Ok(res)
...
impl GeosGeometry {
    ...
    pub fn from_linestring(ln: &LinestringGeometry) -> Result<GeosGeometry> {
        ...
        unsafe {
            let handle = GEOS_init_r();
            let coords = make_coords(handle, ln.lonlats.len(), ln.lonlats.iter().map(|l| { l.to_xy(true) }));
            let geometry = GEOSGeom_createLineString_r(handle, coords);
            if geometry.is_null() {
                GEOS_finish_r(handle);
                return Err(Error::new(ErrorKind::Other, "GEOSGeom_createLineString failed"));
            }
            Ok(GeosGeometry{handle,geometry})
        }
    }
    ...
    pub fn from_complicatedpolygon(cp: &ComplicatedPolygonGeometry) -> Result<GeosGeometry> {
        unsafe {
            ...
            if geometry.is_null() {
                GEOS_finish_r(handle);
                return Err(Error::new(ErrorKind::Other, "GEOSGeom_createCollection_r failed"));
            }
    ...
    pub fn wkb(&self) -> Result<Vec<u8>> {
        unsafe {
            write_wkb(self.handle, self.geometry)
        }
    }
