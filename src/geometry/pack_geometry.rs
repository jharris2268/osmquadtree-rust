...
fn pack_linestring_geometry(pst: &mut Box<PackStringTable>, geom: &LinestringGeometry) -> Result<Vec<u8>> {
    ...
    match geom.z_order {
        None => {}
        Some(z) => { pack_value(&mut res, 12, zig_zag(z)); },
    }
    pack_data(&mut res, 13, &pack_delta_int(geom.lonlats.iter().map(|l| { l.lon as i64})));
    pack_data(&mut res, 14, &pack_delta_int(geom.lonlats.iter().map(|l| { l.lat as i64})));
...
fn pack_simple_polygon_geometry(pst: &mut Box<PackStringTable>, geom: &SimplePolygonGeometry) -> Result<Vec<u8>> {
    let mut res = Vec::with_capacity(95 + 10 * geom.tags.len() + 25*geom.refs.len());
...
fn pack_ringpart(rp: &RingPart) -> Result<Vec<u8>> {
    ...
    pack_data(&mut res, 3, &pack_delta_int(rp.lonlats.iter().map(|l| { l.lon as i64 })));
    pack_data(&mut res, 4, &pack_delta_int(rp.lonlats.iter().map(|l| { l.lat as i64 })));
    if rp.is_reversed {
...
fn pack_complicated_polygons_geometry(pst: &mut Box<PackStringTable>, geom: &ComplicatedPolygonGeometry) -> Result<Vec<u8>> {
...
pub fn pack_geometry_block(gb: &GeometryBlock) -> Result<Vec<u8>> {
    ...
    let mut res = Vec::with_capacity(
        ...
        data_length(2, points_group.len())+ data_length(2, linestrings_group.len())+
        data_length(2, simple_polygons_group.len()) +data_length(2, complicated_polygons_group.len()) +
        20);
