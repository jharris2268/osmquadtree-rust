use crate::elements::{Block, Element, Quadtree};
...
impl Block for GeometryBlock {
    ...
    fn add_object(&mut self, ele: Element) -> Result<()> {
        match ele {
            Element::PointGeometry(p) => {
                self.points.push(p);
                Ok(())
            }
            Element::LinestringGeometry(l) => {
                self.linestrings.push(l);
                Ok(())
            }
            Element::SimplePolygonGeometry(sp) => {
                self.simple_polygons.push(sp);
                Ok(())
            }
            Element::ComplicatedPolygonGeometry(cp) => {
