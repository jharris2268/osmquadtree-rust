...
pub fn write_ring<W: Write, Iter: Iterator<Item=XY>>(w: &mut W, ln: usize, iter: Iter) -> Result<()> {
...
fn prep_wkb_alt(srid: Option<u32>, ty: u32, ln: u32) -> Result<Vec<u8>> {
    let mut res = if ln==0 { Vec::new() } else { Vec::with_capacity(1+4+(if srid.is_none() { 4 } else { 0 })+ln as usize) };
...
impl AsWkb for geo::Polygon<f64> {
    ...
    fn as_wkb(&self, srid: Option<u32>) -> Result<Vec<u8>> {
        let mut res = prep_wkb_alt(srid,1,4*(1+self.interiors().len() as u32)+8*poly_num_points(self))?;
...
fn geos_to_wkb<'a>(geom: &geos::Geometry<'a>) -> Result<Vec<u8>> {
    match geom.to_wkb() {
        ...
        Ok(w) => {
            let res: Vec<u8> = w.into();
            Ok(res)
        }
    }
}

impl<'a> AsWkb for geos::Geometry<'a> {
    fn as_wkb(&self, srid: Option<u32>) -> Result<Vec<u8>> {
        match srid {
            None => geos_to_wkb(self),
            Some(srid) => {
                let mut g2 = Geom::clone(self);
                g2.set_srid(srid as usize);
                geos_to_wkb(&g2)
            }
        }
...
trait AsWkbValidated {
    fn as_wkb_validated(&self, srid: Option<u32>) -> Result<(Option<String>,Vec<u8>)>;
}

impl<'a> AsWkbValidated for geos::Geometry<'a> {
    
    fn as_wkb_validated(&self, _srid: Option<u32>) -> Result<(Option<String>,Vec<u8>)> {
        println!("{:?}", self.is_valid_reason());
