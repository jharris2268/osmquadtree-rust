use crate::elements::{Relation,Way,Tag,ElementType};
use crate::geometry::{OtherData,Timings,WorkingBlock,GeometryStyle};
use crate::geometry::style::{RelationTagSpec, OpType};
                    ...
                    for m in &rel.members {
                        if m.mem_type == ElementType::Way {
                            match self.pending.get_mut(&m.mem_ref) {
                                None => { self.pending.insert(m.mem_ref, vec![(i,val.clone())]); },
                                Some(xx) => { xx.push((i,val.clone())); }
                            }
                        }
                    }
                    added=true;
    ...
    where T: CallFinish<CallType=WorkingBlock, ReturnType=Timings> + ?Sized
{
    ...
    fn finish(&mut self) -> std::io::Result<Timings> {
        ...
        let m = format!("added {} tags to {} ways from {} rels. {} ways not found", self.num_tags, self.num_ways, self.num_rels, self.pending.len());
