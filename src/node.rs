...
impl Node {
    pub fn new(id: i64, changetype: u64) -> Node {
        Node{common: common::Common::new(id, changetype), lon: 0, lat: 0}
    }
    
    pub fn read(changetype: u64, strings: &Vec<String>, data: &[u8], minimal: bool) -> Result<Node> {
        
        let tt = read_pbf::read_all_tags(&data,0);
        let cc = common::Common::read(changetype, &strings, &tt, minimal)?;
    ...
    pub fn pack(&self, _prep_strings: &mut Box<common::PackStringTable>, _include_qts: bool) -> Result<Vec<u8>> {
