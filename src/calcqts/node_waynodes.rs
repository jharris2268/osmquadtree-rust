...
pub fn make_packwaynodescomb<T: CallFinish<CallType=Vec<u8>,ReturnType=Timings>>(out: Box<T>) -> Box<impl CallFinish<CallType=NodeWayNodeCombTile,ReturnType=Timings>> {
...
pub fn write_waynode_sorted_resort(waynodevals: WayNodeVals, outfn: &str) -> String {
    ...
    let wv = CallbackSync::new(Box::new(WrapWriteFileVec::new(WriteFile::new(&waynodesfn, HeaderType::None))),4);
...
fn read_way_node_tiles_vals(pos: &mut u64, tile: i64, vals: &Vec<Vec<u8>>, minw: i64, maxw: i64) -> Result<WayNodeTile> {
    ...
    for (i,v) in vals.iter().enumerate() {
        ...
        if fb.block_type != "WayNodes" {
            return Err(Error::new(ErrorKind::Other, format!("wrong block type {}", fb.block_type)));
        }
...
pub fn write_nodewaynode_file(nodewaynodes: NodeWayNodes, outfn: &str) -> NodeWayNodes {
    ...
    let wvs = CallbackSync::new(Box::new(WrapWriteFile::new(WriteFile::new(&waynodesfn,HeaderType::None))),4);
...
fn read_waynodeways_inmem<T: CallFinish<CallType=NodeWayNodeCombTile,ReturnType=Timings>>(infn: &str, waynodevals: WayNodeVals, eqt: Box<T>, minw: i64, maxw: i64, msg: &str, numchan: usize) -> Timings {
    if numchan == 0 {
        ...
        for c in combines {
            ...
            converts.push(Box::new(Callback::new(make_convert_minimal_block_parts(false, true,false,false,c2))));
        }
...
pub fn read_nodewaynodes<T: CallFinish<CallType=NodeWayNodeCombTile,ReturnType=Timings>>(nodewaynodes: NodeWayNodes, eqt: Box<T>, minw: i64, maxw: i64, msg: &str, numchan: usize) -> Timings {
    match nodewaynodes {
