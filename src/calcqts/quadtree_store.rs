...
impl QuadtreeTileInt {
    pub fn new(off: i64) -> QuadtreeTileInt {
        QuadtreeTileInt {
            off: off,
            valsa: vec![0u32; WAY_SPLIT_VAL],
            valsb: vec![0u16; WAY_SPLIT_VAL],
            count: 0,
        }
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (i64, Quadtree)> + '_ {
        ...
        self.valsa
            ...
            .zip(&self.valsb)
            .enumerate()
            .map(move |(i, (a, b))| (o + (i as i64), Quadtree::new(unpack_val(*a, *b))))
            .filter(|(_, v)| v.as_int() >= 0)
    }
}
...
impl QuadtreeGetSet for QuadtreeSplit {
    ...
    fn set(&mut self, id: i64, qt: Quadtree) {
        ...
        if !self.tiles.contains_key(&idt) {
            self.tiles
                .insert(idt, Box::new(QuadtreeTileInt::new(idt << WAY_SPLIT_SHIFT)));
        }
        if self.tiles.get_mut(&idt).unwrap().set(idi, qt.as_int()) {
            //x, y, z) {
            self.count += 1;
        }
    }
